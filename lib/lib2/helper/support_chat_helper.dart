...
class SupportChatHelper {
  ...
  static String chatTimeCalculation(String dateTime) {
    DateTime messageTime =
        DateFormat('yyyy-MM-dd').parse(dateTime, false).toLocal();
  ...
  static bool isMessageSameDate(String dateTime1, String? dateTIme2) {
    if (dateTIme2 != null) {
      DateTime messageTime =
          DateFormat('yyyy-MM-dd').parse(dateTime1, false).toLocal();
      DateTime preMessageTime =
          DateFormat('yyyy-MM-dd').parse(dateTIme2, false).toLocal();
