...
class RefundRequestController extends GetxController implements GetxService {
  ...
  void getParcelRefundReasonList() async {
    Response response =
        await refundRequestServiceInterface.getParcelRefundReasonList();
  ...
  void sendRefundRequest(
      ...
      required double productApproximatePrice}) async {
    isLoading = true;
    update();
    ...
    if (response.statusCode == 200) {
      ...
      await Get.bottomSheet(
          RefundRequestSendSuccessBottomSheet(parcelId: tripId));
